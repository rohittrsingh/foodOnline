...
The project used a ready-made FoodBakery template from themeforest customized as the goal was to focus on the backend. The project uses Postgres Database The project is deployed on ubuntu Linode with Nginx and Gunicorn The project contains:
HOME page
...
REGISTER page
    ...
    Restaurant registration form - when the restaurant owner wants to register, the admin needs to verify and approve the owner's License Certificate to be registered. Custom registration form when the user registers, the activation form will be sent to the user which he needs to verify to get registered
